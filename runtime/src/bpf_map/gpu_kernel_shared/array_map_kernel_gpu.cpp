...
array_map_kernel_gpu_impl::try_initialize_for_agent_and_get_mapped_address()
{
	if (kernel_map_id <= 0) {
		...
		const auto host_ptr = reinterpret_cast<uintptr_t>(value_data.data());
		...
		SPDLOG_INFO(
			"Mapped GPU memory for kernel-gpu shared array map (userspace fallback): host_ptr={:x} dev_ptr={:x}",
			host_ptr, (uintptr_t)dev_ptr);
